...
def downloadFromURL(url, local_path):
    urllib.request.urlretrieve(url, local_path)# python3
    # python2
    # file = urllib2.urlopen(url).read()
    # open(local_path,"wb").write(file)
...
def run_inference_on_image(image):
    ...
    image_data = tf.io.gfile.FastGFile(image, 'rb').read()
...
if __name__ == '__main__':
    startTime = time.time()
    print('FUNCTION ENTRY TIMESTAMP:%d' % round(startTime * 1000))
    # print(event)
    # print(context)
    ...
    if(os.path.exists("/tmp/imagenet/classify_image_graph_def.pb") is False):
        downloadFromURL(
    ...
    endTime = time.time()
    print('FUNCTION END TIMESTAMP:%d' % round(endTime * 1000))
    print('FUNCTION COST(ms):%d' % round((endTime-startTime) * 1000))
    print(json.dumps({ "return":strResult }))
    # return objRet