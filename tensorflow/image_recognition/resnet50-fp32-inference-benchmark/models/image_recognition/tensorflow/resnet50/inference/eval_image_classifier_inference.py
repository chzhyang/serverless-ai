...
class eval_classifier_optimized_graph:
    ...
    def __init__(self):
        ...
        arg_parser.add_argument("--warmup-steps", type=int, dest="warmup_steps", default=1,
                                help="number of warmup steps")
        arg_parser.add_argument("--steps", type=int, default=50,
                                help="number of steps", dest="steps")
        ...
        arg_parser.add_argument(
            ...
            type=int, default=28)
        arg_parser.add_argument("--model-optimize", dest="model_optimize",
                                help="mode optimize for inference",
                                type=bool, default=False)

        self.args = arg_parser.parse_args()
        # validate the arguements
        self.validate_args()
        print("model_optimize: ", self.args.model_optimize)
        ...
        if self.args.results_file_path:
            ...
            with open(self.args.results_file_path, "a") as fp:
                for filename, expected_label, top_prediction in zip(filenames, labels, top_predictions):
                    fp.write("{},{},{}\n".format(
                        filename, expected_label, top_prediction))
    ...
    def run(self):
        ...
        with data_graph.as_default():
            if (self.args.data_location):
                ...
                input_shape = [self.args.batch_size,
        ...
        with infer_graph.as_default():
            ...
            if self.args.model_optimize == True:
        ...
        print('readmodel:   %.6f sec' % (t_readmodel-t_start))
        if self.args.model_optimize == True:
            print('optimize:    %.6f sec' % (t_optimize-t_readmodel))
            print('loadmodel:   %.6f sec' % (t_loadmodel-t_optimize))
        else:
            print('loadmodel:   %.6f sec' % (t_loadmodel-t_readmodel))
        print('total model: %.6f sec' % (t_end-t_start))
        ...
        infer_sess = tf.compat.v1.Session(
            graph=infer_graph, config=infer_config)
        ...
        if (not self.args.accuracy_only):
            ...
            while num_remaining_images >= self.args.batch_size and iteration < total_run:
                ...
                tf_filenames = None
                np_labels = None
                t_infer_iter_start = time.perf_counter()
                if self.args.results_file_path:
                ...
                predictions = infer_sess.run(output_tensor, feed_dict={
                                             input_tensor: image_np})
                t_infer_iter_end = time.perf_counter()
                ...
                print('Iteration %d: %.6f sec' %
                      (iteration, t_infer_iter_end - t_infer_iter_start))
                # if iteration > warm_up_iteration:
                #     total_time += time_consume
                times.append(t_infer_iter_end - t_infer_iter_start)
            t_infer_end = time.perf_counter()
            time_average = total_time / (iteration - warm_up_iteration)
            avg = (t_infer_end - t_infer_start - times[0])/float(total_run-1)
            print('Average time: %.6f sec' % (avg))
            sum = 0.0
            for j in range(1, iteration):
                sum = sum + times[j]
            avg = sum / float(total_run-1)
            print('Average time: %.6f sec' %
                  (avg))
            # print('Batch size = %d' % self.args.batch_size)
            if (self.args.batch_size == 1):
                print('Latency: %.3f ms' % (avg * 1000))
            # print throughput for both batch size 1 and 128
            print('Throughput: %.3f images/sec' %
                  (self.args.batch_size / avg))
    ...
    def validate_args(self):
        ...
        if not self.args.data_location:
            if self.args.accuracy_only:
                raise ValueError(
                    "You must use real data for accuracy measurement.")
