...
class ComposePostHandler : public ComposePostServiceIf {
 ...
 private:
  ...
  std::vector<Media> _ComposeMediaHelper(
      int64_t req_id, const std::vector<int64_t> &media_ids,
      const std::vector<std::string> &media_types,
      const std::vector<std::string> &media_data_list,
      const std::map<std::string, std::string> &carrier);
...
std::vector<Media> ComposePostHandler::_ComposeMediaHelper(
  ...
  try {
    media_client->ComposeMedia(_return_media, req_id, media_ids, media_types,
                               media_data_list, writer_text_map);
  } catch (...) {
...
void ComposePostHandler::ComposePost(
  ...
  auto media_future = std::async(
      std::launch::async, &ComposePostHandler::_ComposeMediaHelper, this,
      req_id, media_ids, media_types, media_data_list, writer_text_map);
  ...
  //   LOG(info) << "media.media_data: " << media.media_data;
  // }
