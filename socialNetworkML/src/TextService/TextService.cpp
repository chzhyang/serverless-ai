...
int main(int argc, char *argv[]) {
  ...
  if (load_config_file("config/service-config.json", &config_json) == 0) {
    ...
    int text_filter_keepalive = config_json["text-filter-service"]["keepalive_ms"];
    ...
    ClientPool<ThriftClient<TextFilterServiceClient>> text_filter_pool(
        "text-filter-service", text_filter_addr, text_filter_port, 0, 
        text_filter_conns, text_filter_timeout, text_filter_keepalive, config_json);

    std::shared_ptr<TServerSocket> server_socket = get_server_socket(config_json, "0.0.0.0", port);
    TThreadedServer server(
        std::make_shared<TextServiceProcessor>(std::make_shared<TextHandler>(
            &url_client_pool, &user_mention_pool, &text_filter_pool)),
        server_socket,
