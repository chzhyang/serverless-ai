...
void TextHandler::ComposeText(
  ...
  // text-filter-service
  auto text_filter_future = std::async(std::launch::async, [&]() {
    auto text_filter_span = opentracing::Tracer::Global()->StartSpan(
        "text_filter_client", {opentracing::ChildOf(&span->context())});

    std::map<std::string, std::string> text_filter_writer_text_map;
    TextMapWriter text_filter_writer(text_filter_writer_text_map);
    opentracing::Tracer::Global()->Inject(text_filter_span->context(),
                                          text_filter_writer);
    ...
    bool _return_text_filter;
    auto text_filter_client = text_filter_client_wrapper->GetClient();
    try {
      _return_text_filter = text_filter_client->TextFilter(
          req_id, text, text_filter_writer_text_map);
    } catch (...) {
      LOG(error) << "Failed to upload texts to text-filter-service";
      _text_filter_client_pool->Remove(text_filter_client_wrapper);
      throw;
    }

    _text_filter_client_pool->Keepalive(text_filter_client_wrapper);
    return _return_text_filter;
  });
