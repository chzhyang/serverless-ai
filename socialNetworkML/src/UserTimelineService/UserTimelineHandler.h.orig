...
void UserTimelineHandler::ReadUserTimeline(
  ...
  try {
    for (auto &post : _return) {
      // LOG(info) << "post_id: " << post.post_id;
      for (auto &media : post.media) {
        // LOG(info) << "compressed media.media_data: " << media.media_data;
        std::string binary_media =
            Gzip::decompress(Base64::decode(media.media_data));
        media.media_data = Base64::encode(
            reinterpret_cast<const unsigned char *>(binary_media.c_str()),
            binary_media.length());
        // LOG(info) << "decompressed media.media_data: " << media.media_data;
      }
    }
  } catch (...) {
    LOG(error) << "Failed to decompress images";
    throw;
  }
