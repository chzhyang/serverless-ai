local _M = {}
local k8s_suffix = ".deathstarbench-social-network.svc.cluster.local"
...
function _M.ComposePost()
  ...
  if (not _StrIsEmpty(post.media_ids) and not _StrIsEmpty(post.media_types) and not _StrIsEmpty(post.media_data_list)) then
    status, ret = pcall(client.ComposePost, client,
        req_id, post.username, tonumber(post.user_id), post.text,
        cjson.decode(post.media_ids), cjson.decode(post.media_types), cjson.decode(post.media_data_list),
