...
class ComposePostServiceIf {
 public:
  ...
  virtual void ComposePost(const int64_t req_id, const std::string& username, const int64_t user_id, const std::string& text, const std::vector<int64_t> & media_ids, const std::vector<std::string> & media_types, const std::vector<std::string> & media_data_list, const PostType::type post_type, const std::map<std::string, std::string> & carrier) = 0;
};
...
class ComposePostServiceNull : virtual public ComposePostServiceIf {
 public:
  ...
  void ComposePost(const int64_t /* req_id */, const std::string& /* username */, const int64_t /* user_id */, const std::string& /* text */, const std::vector<int64_t> & /* media_ids */, const std::vector<std::string> & /* media_types */, const std::vector<std::string> & /* media_data_list */, const PostType::type /* post_type */, const std::map<std::string, std::string> & /* carrier */) {
    return;
  }
};

typedef struct _ComposePostService_ComposePost_args__isset {
  _ComposePostService_ComposePost_args__isset() : req_id(false), username(false), user_id(false), text(false), media_ids(false), media_types(false), media_data_list(false), post_type(false), carrier(false) {}
  bool req_id :1;
  bool username :1;
  bool user_id :1;
  bool text :1;
...
class ComposePostServiceMultiface : virtual public ComposePostServiceIf {
 ...
 public:
  void ComposePost(const int64_t req_id, const std::string& username, const int64_t user_id, const std::string& text, const std::vector<int64_t> & media_ids, const std::vector<std::string> & media_types, const std::vector<std::string> & media_data_list, const PostType::type post_type, const std::map<std::string, std::string> & carrier) {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->ComposePost(req_id, username, user_id, text, media_ids, media_types, media_data_list, post_type, carrier);
    }
    ifaces_[i]->ComposePost(req_id, username, user_id, text, media_ids, media_types, media_data_list, post_type, carrier);
  }
