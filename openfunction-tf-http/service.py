...
def _cache_model(infer_graph):
    ...
    infer_sess = tf.compat.v1.Session(graph=infer_graph)
    infer_sess.run(output_tensor, feed_dict={
...
start = datetime.now()
print("Start load: ", start, flush=True)
infer_graph = _load_model()
end = datetime.now()
print("End load: ", end, flush=True)
print("load time(s): ", (end - start).microseconds/1000000, flush=True)

input_tensor = infer_graph.get_tensor_by_name(INPUT_TENSOR)
output_tensor = infer_graph.get_tensor_by_name(OUTPUT_TENSOR)

start = datetime.now()
print("Start cache: ", start, flush=True)
_cache_model(infer_graph)
end = datetime.now()
print("End cache: ", end, flush=True)
print("Cache time(s): ", (end - start).microseconds/1000000, flush=True)
...
def _get_labels_for_ids(labels, ids, ids_are_one_indexed=False):
    """Get the human-readable labels for given ids"""
    return [labels[str(x + int(ids_are_one_indexed))] for x in ids]


def _get_top_predictions(results, lables_path, ids_are_one_indexed=False, preds_to_print=5):
    """Given an array of mode, graph_name, predicted_ID, print labels"""
    labels = _get_labels(lables_path)
    predictions = []
    for result in results:
        pred_ids = _top_predictions(result, preds_to_print)
        pred_labels = _get_labels_for_ids(
            labels, pred_ids, ids_are_one_indexed)
        predictions.append(pred_labels)
...
def run_inference(data_location, lables_path, num_top_preds):
    """
    Read and preprocess the image, returns human-readable predictions.
    Preprocess the image to tensor which can be accepted by tensorflow, 
    then run inference, and get human-readable predictions lastly.
    ...
    predictions = infer_sess.run(output_tensor, feed_dict={
        input_tensor: image})
    print("Goto get_top_predictions()", flush=True)
    predictions_labels = _get_top_predictions(
        predictions, lables_path, False, num_top_preds)
...
def _data_preprocessing(data_location, output_height, output_width, num_channels, batch_size):
    """
    ...
    with data_graph.as_default():
        ...
        with tf.compat.v1.Session(graph=data_graph) as sess:
            print("1 ", flush=True)
            image_np = images.eval()
        # coord = tf.compat.v1.train.Coordinator()  # 创建一个协调器，管理线程
        # threads = tf.compat.v1.train.start_queue_runners(
        #     sess=data_sess, coord=coord)  # 启动QueueRunner, 此时文件名队列已经进队
        # image = data_sess.run(images)
        print(type(image_np), flush=True)
        print("image_np: ", image_np, flush=True)
        # coord.join(threads)
        print("0 ", flush=True)
    return image_np
